        ...
        switch (t.getType()) {
            ...
            case Token.NUMBER:
                result = new PropertyValue(
                        CSSPrimitiveValueExtension.CSS_NUMBER,
            ...
            case Token.PERCENTAGE:
                result = new PropertyValue(
                        CSSPrimitiveValueExtension.CSS_PERCENTAGE,
            ...
            case Token.EMS:
                result = new PropertyValue(
                        CSSPrimitiveValueExtension.CSS_EMS,
                        sign*Float.parseFloat(extractNumber(t)),
                        sign(sign) + getTokenValue(t));
                next();
                skip_whitespace();
            ...
            case Token.EXS:
                result = new PropertyValue(
                        CSSPrimitiveValueExtension.CSS_EXS,
                        sign*Float.parseFloat(extractNumber(t)),
                        sign(sign) + getTokenValue(t));
                next();
                skip_whitespace();
            ...
            case Token.EHS:
                result = new PropertyValue(
                        CSSPrimitiveValueExtension.CSS_EHS,
                        sign*Float.parseFloat(extractNumber(t)),
                        sign(sign) + getTokenValue(t));
                next();
                skip_whitespace();
            ...
            case Token.CM:
                result = new PropertyValue(
                        CSSPrimitiveValueExtension.CSS_CM,
                        sign*Float.parseFloat(extractNumber(t)),
                        sign(sign) + getTokenValue(t));
                next();
                skip_whitespace();
            ...
            case Token.PT:
                result = new PropertyValue(
                        CSSPrimitiveValueExtension.CSS_PT,
                        sign*Float.parseFloat(extractNumber(t)),
                        sign(sign) + getTokenValue(t));
                next();
                skip_whitespace();
            ...
            case Token.PC:
                result = new PropertyValue(
                        CSSPrimitiveValueExtension.CSS_PC,
                        sign*Float.parseFloat(extractNumber(t)),
                        sign(sign) + getTokenValue(t));
                next();
                skip_whitespace();
            ...
            case Token.IDENT:
                ...
                result = new PropertyValue(
                        CSSPrimitiveValueExtension.CSS_IDENT,
                        value,
                        value);
                next();
                skip_whitespace();
            ...
            case Token.URI:
                result = new PropertyValue(
                        CSSPrimitiveValueExtension.CSS_URI,
                        getTokenValue(t),
                        getRawTokenValue());
                next();
                skip_whitespace();
...
//    ;
    private PropertyValue function() throws IOException {
        ...
        if (t == Token.TK_FUNCTION) {
            ...
            }  else if (f.equals("cmyka(")) {
                if (! isSupportCMYKColors()) {
                    throw new CSSParseException(
                            "The current output device does not support CMYK colors", getCurrentLine());
                }
                //in accordance to http://www.w3.org/TR/css3-gcpm/#cmyk-colors
                result = new PropertyValue(createCMYKAColorFromFunction(params));
            } else {
    ...
    private FSCMYKColor createCMYKAColorFromFunction(List params) {
        if (params.size() != 5) {
            throw new CSSParseException(
                    "The cmyk() function must have exactly four parameters",
                    getCurrentLine());
        ...
        float[] colorComponents = new float[5];

        for (int i = 0; i < params.size(); i++) {
            colorComponents[i] = parseCMYKColorComponent((PropertyValue)params.get(i), (i+1)); //Warning on the truncation?
        }

        return new FSCMYKColor(colorComponents[0], colorComponents[1], colorComponents[2], colorComponents[3], colorComponents[4]);
    ...
    private float parseCMYKColorComponent(PropertyValue value, int paramNo) {
        ...
        } else if (type == CSSPrimitiveValueExtension.CSS_PERCENTAGE) {
    ...
    private FSRGBColor createRGBColorFromFunction(List params) {
        ...
        for (int i = 0; i < params.size(); i++) {
            ...
            short type = value.getPrimitiveType();
            if (type != CSSPrimitiveValueExtension.CSS_PERCENTAGE &&
